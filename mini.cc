...
WindowManager::WindowManager(int argc, char** argv)
{
  ...
  signal(SIGCHLD, sigHandler); 
	...
	if (window_placement.compare("random")==0) random_window_placement = true;
	else if (window_placement.compare("mouse")==0) random_window_placement = false;
	else random_window_placement = false; 
  ...
  if (!font) 
  { 
    std::cerr << "The default font cannot be found, exiting..." << std::endl; 
    exit(-1); 
  }
...
void WindowManager::addClient(Window w)
{
  ...
  reparentClient(c);
	...
	if (getWMState(c->window) == IconicState) iconifyClient(c);
	else 
	{
		unhideClient(c);
...
{  
  XGrabServer(dpy);
	...
	if(c->trans) XSetInputFocus(dpy, c->trans, RevertToNone, CurrentTime);
...
void WindowManager::handleButtonPressEvent(XEvent *ev)
{
  ...
  if(c && c->has_title)
  {
    ...
    if(!XGrabPointer(dpy, c->frame, False, PointerMotionMask|ButtonReleaseMask, GrabModeAsync, GrabModeAsync, None, move_curs, CurrentTime) == GrabSuccess) return; 
  } 
...
void WindowManager::handleButtonReleaseEvent(XEvent *ev)
{
	...
	{
	  if (ev->xbutton.window==root && ev->xbutton.button==Button3) forkExec(DEFAULT_CMD);
	}
}
...
{ 
  Client* c = findClient(ev->xclient.window);
	...
	if(c)
		handleClientClientMessage(&ev->xclient, c);
}
...
{ 
  Client* c = findClient(ev->xcolormap.window);
	...
	if(c)
		handleClientColormapChange(&ev->xcolormap, c);
}
...
// Dunno where I ripped this from function. Kudos to the author whoever he is!
...
void WindowManager::handleClientConfigureRequest(XConfigureRequestEvent *ev, Client* c) 
{
  ...
  if (ev->value_mask & (CWWidth|CWHeight)) setClientShape(c);
}
...
void WindowManager::handleClientClientMessage(XClientMessageEvent *ev, Client* c) 
{
  if (ev->message_type == atom_wm_change_state && ev->format == 32 && ev->data.l[0] == IconicState) 
    iconifyClient(c);
}
...
void WindowManager::handleClientColormapChange(XColormapEvent *ev, Client* c) 
{
  if (ev->c_new) 
  {
    c->cmap = ev->colormap;
...
void WindowManager::handleClientMotionNotifyEvent(XMotionEvent *ev, Client* c) 
{
  ...
  int theight = getClientTitleHeight(c);
	...
	if((ev->state & Button1Mask) && (focused_client == c))
	{
	  ...
	  if(WIRE_MOVE) drawClientOutline(c);
	  ...
	  if(EDGE_SNAP)
	  {
	    ...
	    if(nx == xres - c->width) nx = xres - c->width + 1;		
	    else if(nx == 0) nx = -1;
	    ...
	    if( (nx + c->width >= xres - SNAP) && (nx + c->width <= xres) ) nx = xres - c->width;
	    ...
	    {
	      if((ny + c->height >= yres - SNAP) && (ny + c->height <= yres)) ny = yres - c->height;
	      else if((ny - theight <= SNAP) && (ny - theight >= 0)) ny = theight;
	    }
	  }
	...
	else if(ev->state & Button3Mask)
	{
		...
		else
		{
          ...
          if (c->height > c->size->max_height) c->height = c->size->max_height;
					...
					if(c->width>xres) c->width=xres;
					if(c->height>yres) c->height=yres;
				} 
...
void WindowManager::hideClient(Client* c) 
{
  if (!c->ignore_unmap) c->ignore_unmap++; 
	...
	if(c->has_focus) setClientFocus(c, false);
...
void WindowManager::iconifyClient(Client* c) 
{
  if (!c->ignore_unmap) c->ignore_unmap++; 
...
void WindowManager::maximizeClient(Client* c) 
{
  ...
  if(c->trans) return;
	...
	else 
	{
		...
		if(c->is_shaded) c->is_shaded=false;
	}
...
void WindowManager::redrawClient(Client* c) 
{
  if (!c->has_title) return;
	...
	if(c->has_focus) gc = border_gc;
	else gc = unfocused_gc;
  ...
  XDrawLine(dpy, c->title, gc, 0, theight - BW + BW/2, c->width, theight - BW + BW/2);
	...
	if(c->has_focus) gc = focused_title_gc;
...
void WindowManager::reparentClient(Client* c) 
{
  ...
  int theight = getClientTitleHeight(c);
	...
	if(c->border_width) { b_w = c->border_width; XSetWindowBorderWidth(dpy, c->window, 0); } else { b_w = BW; }
  ...
  unsigned int nwins, i;
	...
	if(client_list.size())
	{
		...
		for (i = 0; i < nwins; i++)
		{
			...
			if(c)
			{
				if((c->has_title && c->is_iconified) == false)
